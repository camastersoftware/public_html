...
class OrgTypes extends BaseController
{
    public function __construct()
    {
        ...
        $this->section="Organisation Type";
	...
	public function index()
	{
        ...
        $pageTitle="Organisation Type List";
        ...
        $resultArr = $this->MorganisationType->where('status', 1)
                    ->findAll();

        $this->data['resultArr']=$resultArr;

        return view('super_admin/masters/org_types', $this->data);
	}
	
	public function add()
	{
	    $organisation_type_name=$this->request->getPost('organisation_type_name');
	    
	    $dataArray = [
            'organisation_type_name' => $organisation_type_name,
            'status' => 1,
            'createdBy' => $this->adminId,
            'createdDatetime' => $this->currTimeStamp
        ];
	    
        if($this->MorganisationType->save($dataArray)){
        ...
        return redirect()->route('superadmin/org_types');
	}
	
	public function edit()
	{
	    $organisation_type_id=$this->request->getPost('organisation_type_id');
	    $organisation_type_name=$this->request->getPost('organisation_type_name');
