                ...
                <div class="box-body p-10">
                    ...
                    <div class="tab-content tabcontent-border p-5" id="myTabContent">
                        ...
                        <?php for($w=1; $w<13; $w++): ?>
                            ...
                            <!-- Tab panes -->
                            <div class="tab-pane fade table-responsive mis_ddf_div<?= $a ?> <?php if($a==$currMth): ?>show active<?php endif; ?>" id="<?php echo $mth_nm; ?>_tab" role="tabpanel" aria-labelledby="<?php echo $mth_nm; ?>-tab">
                                <div class="row">
                                    <?php if(isset($misAssignArr[$a])): ?>
                                        <?php $ddfCounter=0; ?>
                                        <?php $dueDateForListCount=0; ?>
                                        <?php $totalAssignCount=$totalFiledCount=$totalPendingCount=0; ?>
                                        <?php if(!empty($dueDateForArr)): ?>
                                            <?php $dueDateForListCount=count($dueDateForArr); ?>
                                            <?php foreach($dueDateForArr AS $ddfId=>$ddfName): ?>
                                                <?php $ddfCounter++; ?>
                                                <?php if(!empty($misAssignArr[$a][$ddfId])): ?>
                                                    <div class="offset-md-1 col-md-10">
                                                        <div class="tab-content tabcontent-border p-5" id="myTabContent">
                                                            <div class="tab-pane fade show active">
                                                                <table id="tablepress-2" class="tablepress tablepress-id-2 custom-table dataTable-act no-footer mis_ddf_table mis_ddf_tbl<?= $a ?>">
                                                                    <thead>
                                                                        <tr class="row-1">
                                                                            <th class="column-2 grp_hd_clr" colspan="8">Due Date For - <?= $ddfName; ?></th>
                                                                        </tr>
                                                                        <tr class="row-1">
                                                                    ...
                                                                    <tbody class="row-hover">
                                                                        <?php $sr=1; ?>
                                                                        <?php if(!empty($dueDateForArr)): ?>
                                                                            <?php if(isset($cliDueDateForArr[$ddfId])): ?>
                                                                                <?php $cliGrpArray=$cliDueDateForArr[$ddfId]; ?>
                                                                                ...
                                                                                <?php if(!empty($cliGrpArray)): ?>
                                                                                    <?php foreach($cliGrpArray AS $e_grp): ?>
                                                                                        ...
                                                                                        <?php
                                                                                            $assignCount=0;
                                                                                            if(isset($misAssignArr[$a][$ddfId][$client_group_id]))
                                                                                                $assignCount=$misAssignArr[$a][$ddfId][$client_group_id];
                                                                                            else
                                                                                                $assignCount=0;
                                                                                            
                                                                                            $filedCount=0;
                                                                                            if(isset($misFiledArr[$a][$ddfId][$client_group_id]))
                                                                                            {
                                                                                                $misFiledArray=$misFiledArr[$a][$ddfId][$client_group_id];
                                                                                        ...
                                                                                        <?php if($assignCount!=0): ?>
                                                                                            <tr class="row-1 <?php if($allFiled): ?>hasCompleted<?php endif; ?>">
                                                                                                ...
                                                                                                <td class="column-2 text-center" width="100px">
                                                                                                    <a href="<?php echo base_url('gst-position-of-returns-client/'.$e_grp['client_group_id'].'/'.$ddfId.'/'.$mth_nm_tab); ?>" >
                                                                                    ...
                                                                                    ?>
                                                                                    <tr class="row-1">
                                                                                        <td class="column-1" width="50px">
                                                                                            <script>removeOldGrandTotalValues(<?= $a ?>);</script>
                                                                                            <input type="hidden" class="totalAssignCount<?= $a ?>" id="totalAssignCount<?= $a ?>" value="<?= $totalAssignCount ?>" />
                                                                                            <input type="hidden" class="totalFiledCount<?= $a ?>" id="totalFiledCount<?= $a ?>" value="<?= $totalFiledCount ?>" />
                                                                                            <input type="hidden" class="totalPendingCount<?= $a ?>" id="totalPendingCount<?= $a ?>" value="<?= $totalPendingCount ?>" />
                                                                                        </td>
...
<script>
    function removeOldGrandTotalValues(w)
    {
        var mis_ddf_tbl = $('.mis_ddf_tbl'+w).length;
        
        if($(mis_ddf_tbl).length){
            console.log('mis_ddf_tbl', '.mis_ddf_tbl'+w, mis_ddf_tbl);
            $('.totalAssignCount'+w).remove();
            $('.totalFiledCount'+w).remove();
            $('.totalPendingCount'+w).remove();
        }
    }
</script>
<?= $this->endSection(); ?>
...
<?= $this->section('javacript'); ?>
    <script>
        $(document).ready(function(){
            ...
            function attactGrandTotalRow()
            {
                for(m=1; m<13; m++)
                {
                    if($('.mis_ddf_tbl'+m).length)
                    {
                        var totalAssignCount = $('.totalAssignCount'+m).val();
                        var totalFiledCount = $('.totalFiledCount'+m).val();
                        var totalPendingCount = $('.totalPendingCount'+m).val();
                        ...
                        $('.mis_ddf_div'+m+' table.mis_ddf_table').last().find('tbody').append(grandTotalRow);
                    }
                }
            }
        });
